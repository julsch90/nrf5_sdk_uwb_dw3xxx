...
void dw_irq_init(void)
{
    ...
    // TODO: save more power if not using high accuracy -> set false (but maybe inaccurate) // DISABLE WHILE SLEEP ???
...
void wakeup_device_with_io(void)
{
    ...
    // nrf_drv_gpiote_clr_task_trigger(DW3000_CS_Pin);
    ...
    // nrf_drv_gpiote_set_task_trigger(DW3000_CS_Pin);
...
{
    bool status = nrf_drv_gpiote_in_is_set(current_irq_pin);
